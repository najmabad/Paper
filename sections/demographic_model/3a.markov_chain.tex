...
\begin{center}
\begin{tikzpicture}
\node[circle,label={$transition$}] at (2,1.5) {};
\node[circle,label={$probabilities$}] at (2,1) {};
\node[circle] at (2,1) {$(p_{ij})$};
\node[circle] at (-1.5,1) {$\; \forall j \in S$};
\draw[decoration={brace,raise=5pt},decorate]
  (0,0) -- node[left=10pt] {$X_{t+1}= j, $} (0,3);
\draw[decoration={brace,raise=5pt},decorate]
  (0,3) -- node[above=20pt] {$X_t = i, \; \forall i \in S$} (4,3);
\end{tikzpicture}
\end{center}
